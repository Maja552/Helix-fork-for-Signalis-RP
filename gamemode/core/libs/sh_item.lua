	...
	if (CLIENT) then
		...
		function ix.item.PerformInventoryAction(client, action, item, invID, data)
			...
			if (!item.bAllowMultiCharacterInteraction and IsValid(client) and client:GetCharacter()) then
				...
				if (itemPlayerID and itemCharacterID and itemPlayerID == playerID and itemCharacterID != characterID and
					!ix.config.Get("allowMultipleCharacterInteraction", false)
				) then
